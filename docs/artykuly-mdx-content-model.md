...
- **`<ArticleSection label="1. Otwarcie">...</ArticleSection>`** — opcjonalny numerowany układ sekcji w stylu case-study (chip kroku + własny scope dla `MarginNote`). Używać tylko, gdy artykuł ma sens jako numerowana historia — zwykłe artykuły używają zwykłych `##` nagłówków. *Wstawiany przyciskiem w edytorze `/keystatic`.* Artykuły, które używają `ArticleSection`, automatycznie dostają zwijany **spis treści** (domyślnie zwinięty, tuż pod nagłówkiem) zbudowany z etykiet wszystkich sekcji — nic dodatkowo nie trzeba pisać, wystarczą same `<ArticleSection label="...">` (patrz `src/lib/articles.ts` → `getSectionLabels`, `src/components/articles/table-of-contents.tsx`). Artykuły bez `ArticleSection` nie dostają spisu treści.
...
**Znane ograniczenia:**
- `instruktor-tanca-tego-nie-zrozumiesz.mdx` (macierz umiejętności, ręczny grid JSX) to jedyny artykuł bez zarejestrowanego komponentu w Keystatic — zbyt jednostkowy przypadek, żeby się opłacało go modelować. Nie otwierać go w tym edytorze (nieznany JSX może zniknąć przy zapisie); edytować bezpośrednio jako plik (appka GitHub / edytor kodu). `ArticleSection` (case study) jest już bezpieczny do edycji przez `/keystatic`.
